...
installations themselves}, not recipes: an end user does not need to perform an installation
of the scientific software they wish to use. Environment modules are used as the user interface, and automatic
detection of the target system architecture is done. This means that the binary files that make up the optimised
scientific software that the users require are streamed (via CernVM-FS) to the platform as they are accessed. It brings
all the benefits of a container environment without the need for a runtime, and in addition is automatically updated
and customisable for the host.
